...
# Introduction
today i have learned about how to Extract Local State into Redux
under React and Redux.
...
Recall that we wrote all the Redux code so that Redux could control the state management of our React messages app. Now that Redux is connected, we need to extract the state management out of the Presentational component and into Redux. Currently, you have Redux connected, but you are handling the state locally within the Presentational component.

now less take the code below for example and In the Presentational component, first, remove the messages property in the local state. These messages will be managed by Redux. Next, modify the submitMessage() method so that it dispatches submitNewMessage() from this.props, and pass in the current message input from local state as an argument. Because you removed messages from local state, remove the messages property from the call to this.setState() here as well. Finally, modify the render() method so that it maps over the messages received from props rather than state.
